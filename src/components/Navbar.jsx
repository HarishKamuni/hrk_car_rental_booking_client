...
const Navbar = ({ setShowLogin }) => {
  ...
  return (
    ...
    >
      <Link to="/">
        <img src={assets.logo} alt="log" className="h-8" />
      </Link>
      <div
        className={`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16 max-sm:border-t border-borderColor 
      right-0 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8 max-sm:p-4 transition-all duration-300 ease-out z-50
      ...
      >
        ...
        <div className="flex max-sm:flex-col items-start sm:items-center gap-6">
          ...
          <button
            ...
            className="cursor-pointer px-8 py-2 bg-primery hover:bg-primary-dull transition-all text-white rounded-lg"
          >
            Login
          </button>
        </div>
